...
class VizualizerTrade_Module_Bill_Pdf_Deliver extends Vizualizer_Plugin_Module_Pdf
{
    ...
    function execute($params)
    {
        ...
        foreach($billIds as $billId){
            ...
            // タイトルを描画
            $this->text(241, 248, 20, "納　　品　　書", true);

            // 帳票番号を描画
            $this->text(455, 297, 9, "No：".sprintf("%04d", $destCompany->company_id)."-".sprintf("%08d", $bill->quotation_id), true);

            // 作成日を描画
            $this->text(455, 309, 9, "納品日：".date("Y年m月d日", strtotime($bill->delivered_date)), true);
